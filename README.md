...
**마이 응우옌**을 중심으로, 개인의 삶(뚜렷한 "노드")이 제도·미디어·커뮤니티라는
...
## 페르소나: 마이 응우옌 (Mai Nguyen)

- 35세, 베트남 껀터(Cần Thơ) 출신, 결혼이주로 한국 거주 11년차
- 경기도 연천군 백학면 거주. 집 근처에 철책, 남편은 철책 안 농지에서 인삼농사
- 남편이 매일 검문소를 통과해 출근. 주 3일 DMZ 협동조합 카페에서 근무
  (커피 제조, 커피콩빵, 외국어 수업)
- 가족: 남편, 중학생 아들, 초등학생 딸, 함께 사는 시어머니
- 마음속 질문: "내 아이는 베트남 사람일까, 한국 사람일까, 아니면 이 마을 사람일까"
  ...
  client/                 Vite + React + TypeScript
    src/
      ...
      data/
        ...
        letter.ts            마이의 "마지막 편지" 텍스트 (3D 미션 클라이맥스용)
      components/
      ...
      game/                  3D 마을 (three.js, 프레임워크 없이 직접 구현)
        ...
        character.ts            마이 캐릭터(저폴리 프리미티브 조합) + 걷기 애니메이션
...
- 왼쪽 패널: D3-force 네트워크 그래프. 노드 클릭 시 해당 주제로 대화 트리거,
  검색창으로 노드 찾기 가능
- 오른쪽 패널: 메신저 스타일 채팅. 퀵리플라이 4개는 마이의 실제 대사 인용
...
- **미션 흐름**: 우체통 클릭 → (잠금 상태면) "마이와 먼저 이야기해봐요" 안내 →
...
- **마을 레이아웃**: 메인 거리(남북 축) 양옆에 집 10채(왼쪽 5, 오른쪽 5), 각 집은
  진입로로 거리와 연결. 마이의 집이 스폰 지점. 거리 끝에 우체통
